class Company {
    ...
    addEmployee(username, salary, position, department) {
        ...
        let existDepartment = this.departments.find(d => d.name === department);
        if (!existDepartment) {
            existDepartment = {
                name: department,
                employees: [],
                averageSalary: function () {
                    return this.employees.reduce((previousValue, currentValue) => previousValue + currentValue.salary, 0) / this.employees.length;
                }
            };
            this.departments.push(existDepartment)
        }
        existDepartment.employees.push({username, salary, position});
